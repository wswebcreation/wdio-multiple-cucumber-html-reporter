...
class MultipleCucumberHtmlReporter extends events.EventEmitter {
    ...
    determineMetadata(data) {
        ...
        if (bodyValue.app || bodyValue.testobject_app_id || metadata.app) {
            const metaAppName = metadata.app.name || 'No metadata.app.name available';
            const metaAppVersion = metadata.app.version || 'No metadata.app.version available';
            const appPath = (bodyValue.testobject_app_id || metaAppName);
