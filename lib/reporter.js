...
class MultipleCucumberHtmlReporter extends events.EventEmitter {
    ...
    determineMetadata(data) {
        ...
        if (bodyValue.app || bodyValue.testobject_app_id) {
            const appPath = (bodyValue.app || bodyValue.testobject_app_id);
            ...
            metadata.device = desiredCapabilities.deviceName || bodyValue.deviceName || null;
            metadata.platform = {
                name: desiredCapabilities.platformName || bodyValue.platformName || null,
                version: desiredCapabilities.platformVersion || bodyValue.platformVersion || null,
            }

        } else {
            const
                currentBrowserName = bodyValue.browserName;
            const
                currentBrowserVersion = bodyValue.version || bodyValue.browserVersion;
            browser = {
                browser: {
